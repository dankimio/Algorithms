...
// and works in average O(n * log(n)) time complexity. O(n * n) is the worst
// case. depth - recursion depth, leaves - number of leaves in recursion tree
...
// Complexity: O(1)
int quicksort::get_middle_pivot(int from, int to) { return (from + to) / 2; }

// Getting position of last pivot in array in the current bounds
//
// Additional space: O(1)
// Complexity: O(1)
int quicksort::get_last_pivot(int from, int to) { return to; }
...
void quicksort::recursive_call_qsort(int *arr, int from, int to,
                                     pivot_chooser pivot_chooser, int &depth,
                                     int &leaves) {
    if (from < to) {
        int local = depth;
        local++;
